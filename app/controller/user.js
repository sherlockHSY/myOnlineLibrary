...
class UserController extends Controller {
    ...
    async userMsg() {
        ...
        let res1 = await this.app.mysql.query('select user_name,email,password,sex,phone,size from user,storage where user.storage_id = storage.storage_id and user.user_name = ?', [this.ctx.state.user]);
        if (res1.length == 0) {
            this.ctx.status = 400;
            return this.ctx.body = {
                msg: '用户信息获取失败'

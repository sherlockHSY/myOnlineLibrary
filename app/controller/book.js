...
class BookController extends Controller {
    ...
    async uploadToPublic() {
        ...
        let result0 = await this.app.mysql.select('book', {
            columns: ['book_id'],
            orders: [['book_id', 'desc']]
        });
        var id = result0.length === 0 ? 1 : parseInt(result0[0].book_id) + 1;
        ...
        while ((part = await parts()) != null) {
            ...
            if (part.length) {
                ...
                if (part.fieldname === "image") {
                    ...
                    let url = '/public/publicStorage/cover/cover-' + id + '.' + type;
                ...
                if (part.fieldname === "file") {
                    console.log(part)
                    let type = part.mimeType.replace('application/', '');
                    let url = '/public/publicStorage/book/book-' + id + '.' + part.mimeType.replace('application/', '');
        ...
        let result = await this.ctx.service.book.insert(id, insertInfo);
    ...
    async uploadToPrivate() {
        ...
        let result = await this.app.mysql.get('user_storage', {
            user_name: this.ctx.state.user
        })
        console.log(result)
        let storageAddress = result.address;
        let result0 = await this.app.mysql.select('book', {
            columns: ['book_id'],
            orders: [['book_id', 'desc']]
        });
        ...
        while ((part = await parts()) != null) {
            ...
            if (part.length) {
                ...
                if (part.fieldname === "image") {
                    ...
                    let url = storageAddress + '/cover/cover-' + id + '.' + type;
                    insertInfo.cover_address = url;
                    target = path.join(this.config.baseDir, 'app', url);
                ...
                if (part.fieldname === "file") {
                    ...
                    let url = storageAddress + '/book/book-' + id + '.' + type;
                    insertInfo.book_address = url;
                    target = path.join(this.config.baseDir, 'app', url);
        ...
        let result1 = await this.ctx.service.book.insert(id, insertInfo);
    ...
    async showPrivate() {
        ...
        try {
            let result = await this.app.mysql.select('private_book', {
                user_name: this.ctx.state.user
            });
            this.ctx.body = {
                data: result,
    ...
    async showPublic() {
        try {
            let result = await this.app.mysql.select('public_book');
            this.ctx.body = {
                data: result,
                msg: '获取公共图书信息成功'
            }
    ...
    //根据id查看书籍详情
    async showBook(){
        let id = parseInt(this.ctx.query.id);
        //统计详细信息
        let result = await this.app.mysql.get('book',{
            book_id: id
        })
        return result
    ...
    async searchBook() {
        ...
        console.log(keyword)
        //优先根据书名查找 模糊查找
        let result = await this.app.mysql.query("select * from book where book_name like ?",[keyword])
        if(result.length === 0) {
            //查找不到，按作者查找
            let result1 = await this.app.mysql.query("select * from book where author like ?",[keyword])
            if(result1.length === 0) {
                this.ctx.status = 400
    ...
    async update() {
        ...
        while ((part = await parts()) != null) {
            ...
            if (part.length) {
                ...
                if (part.fieldname === "file") {
                    console.log(part)
                    let type = part.mimeType.replace('application/', '');
                    let url = '/public/publicStorage/book/book-' + id + '.' + type;
        ...
        let result = await this.ctx.service.book.update(id, updateInfo);
    ...
    async delete() {
        ...
        var id = parseInt(this.ctx.request.body.book_id);

        let result = await this.ctx.service.book.destroy(id);
